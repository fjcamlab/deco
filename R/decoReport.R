...
decoReport <- function(deco, sub, id = NA, pdf.file = "decoReport.pdf", info.sample = NA,
    ...
    ### Formatting...
    formatObj <- .formattingObj(deco, deco0, analysis, info.size)
    ...
    if (any(id %in% as.character(deco@featureTable[, c("ID")]))) {
        ...
        if (any(!(id %in% id_sus))) {
            message("NOTE: Following IDs have not been found by DECO or they 
                have not been included in NSCA
                (if COMPLETE profiles were removed), so 'Profile' 
...
.plotRepThr <- function(sub, deco, id = NA, print.annot = FALSE) {
    ...
    # Setting up Colors
    col <- vapply(y, function(j) max(which(seq(1, max(y), length.out = 16) <=
            j)), numeric(1))
    ...
    axis(2, at = seq(1, max(y), length.out = 16), labels = round(seq(1, max(y),
        length.out = 16
    ), 0), font = 2, las = 2)
...
plotDECOProfile <- function(deco, id, data, pdf.file = NA, plot.h = FALSE,
                            info.sample = NA, print.annot = TRUE, cex.legend = 1.1, cex.names = 1,
                            cex.samples = 1) {
