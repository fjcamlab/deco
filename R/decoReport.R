...
decoReport <- function(deco, sub, id = NA, pdf.file = "decoReport.pdf",
    ...
    # Open pdf file.
    pdf(
        file = pdf.file, width = 18, height = 12,
        family = "Helvetica-Narrow"
    )

    # Variable used to annotate if print.annot == T
    # and biological context is supplied.
    ...
    j <- rbind(
        analysis, dim(deco@incidenceMatrix)[2],
        round(deco@featureTable[1, c("Repeats")] / deco@featureTable[1, c("FR.Repeats")], digits = 0),
        deco@pos.iter,
        round(deco@pos.iter / deco@featureTable[1, c("Repeats")] / deco@featureTable[1, c("FR.Repeats")] * 100, 2),
        dim(deco@featureTable)[1], min(deco@featureTable$Repeats) - 1,
        deco@q.val, sub$resampleSize
    )

    # Generating colors for all different sample information input by user.
    if (all(is.na(info.sample))) {
        info.sample <- matrix(NA,
            nrow = ncol(deco@incidenceMatrix),
    ...
    color.table <- matrix(
        data = "black", nrow = 3,
        ncol = dim(formatObj$nsc)[2]
    )
    color.table[2, formatObj$nsc[2, ] > 0.05] <- "red"
    textplot(formatObj$nsc,
        col.rownames = "darkblue", cex = 1.7,
        valign = "top", col.data = color.table
    )
    ...
    y <- na.omit(cbind(
        Ranking = seq_len(10),
        formatObj$textF[seq_len(10), colnames(formatObj$textF) %in%
            c("ID", "UpDw", "Profile", "SYMBOL"), drop = FALSE]
    ))
    textplot(
    ...
    textplot(
        valign = "top", object = formatObj$infoSubclass,
        col.data = colors,
        col.rownames = col,
        cex = 3 / (0.25 * (dim(formatObj$infoSubclass)[1] + 3))
    )
    ...
    color.table <- matrix(
        data = "black", ncol = 2,
        nrow = dim(formatObj$samplesSubclass)[1]
    )
    color.table[is.na(formatObj$samplesSubclass[, c("Subclass")]), 1] <- "red"
    formatObj$samplesSubclass[is.na(
        formatObj$samplesSubclass[, c("Subclass")]
    ), c("Subclass")] <- "NoSubclass"
    textplot(formatObj$samplesSubclass,
        valign = "top", cmar = 2,
        mar = c(3, 3, 5, 3), col.colnames = "darkblue",
        col.data = color.table, col.rownames = "darkblue"
    )
    title(
        main = list(paste("Overview\nDECO analysis report"), cex = 2.5, font = 2),
        line = -6, outer = TRUE
    )
    ...
    if (analysis == "Binary") {
        ...
        .plotTau(
            ...
            samplesSubclass = formatObj$samplesSubclass,
            formatObj$color.cluster, labels = symbol, n = 20
        )
    }
    else {
        .plotTau(
            x = list(deco@NSCAcluster$All),
            samplesSubclass = formatObj$samplesSubclass,
            formatObj$color.cluster, labels = symbol, n = 20
        )
    }
    ...
    legend("bottom",
        legend = c("Cumulative"), col = c("darkgoldenrod"), xpd = TRUE, inset = c(0, -0.1),
    ...
    .plotRankingF(
        deco, formatObj, info.size,
        id, analysis, default.layout
    )
    ...
    layout(matrix(1))
    if (analysis == "Binary") {
        .plotOD(deco0, id, ord = list(formatObj$ordG, formatObj$ordS), symbol = symbol, print.annot = print.annot)
        .plotDend(
            list(deco@NSCAcluster$Control$hclustSamp, deco@NSCAcluster$Case$hclustSamp), analysis,
            formatObj$color.cluster, formatObj$samplesSubclass, cex.samples + 0.1
        )
    } else {
        .plotDend(deco@NSCAcluster$All$hclustSamp, analysis, formatObj$color.cluster, formatObj$samplesSubclass,
    ...
    .plotHeatmapDECO(
        deco, formatObj$p.val, id, analysis, symbol, infoS, formatObj$color.cluster, data0,
        cex.legend, cex.names, cex.samples, print.annot,
        info.sample, info.feature, formatObj$samplesSubclass
    )
    ...
    .plotProfileDECO(
        ...
        info.sample, formatObj$infoSubclass, formatObj$samplesSubclass,
    ...
    if (analysis == "Binary") {
        rankingH <- cbind(
            deco@NSCAcluster[[1]]$rankingFeature.h,
...
.plotHeatmapDECO <- function(deco, p.val, id, analysis, symbol, infoS, color.cluster, data0,
    ...
    if (length(p.val) == 1 & p.val[1] > 0.05) {
        ...
        message(
            ...
            c("CASES", "CONTROL")[p.val > 0.05],
            "samples shows NSCA p-value > 0.05."
        )
    }
    while (i %in% seq_len(2)) {
        ...
        if (all(is.na(info.feature))) {
            ...
            for (z in seq_len(length(unlist(apply(info.feature, 2, unique))))) {
                info.feature.color[
                    info.feature == unlist(apply(info.feature, 2, unique))[z]
                ] <- as.character(
                    rep(
                        colorRampPalette(RColorBrewer::brewer.pal(name = "Paired", n = 10))(length(unlist(apply(info.feature, 2, unique))))[z],
            ...
            # Final label matrix to place under samples dendrogram.
            feature.color <- cbind(
                Feature.pattern = infoFpat, Exprs = feature.label, rep("white", length(feature.label)),
        ...
        mtext(
            text = paste("f =", dim(MX)[1], "features for heatmap"),
            side = 4, las = 3, font = 2, cex = 1.3
        )

        # Legends including feature and sample information.
        if (all(!is.na(info.sample))) {
            legend("topleft",
                legend = infoS$ty, col = names(infoS$ty), pch = 15, cex = cex.legend - 0.2,
                title = "Sample info provided by user", bty = "n", xpd = TRUE, inset = c(0, 0.1)
            )
        }
        legend(
            title = "DECO feature-sample subclasses", "topright",
        ...
        if (i == 3) {
            legend("bottomleft",
                legend = c("DE features", "ID provided features"), title = "Features",
                col = c("black", "red"), bty = "n", cex = cex.legend - 0.1, pch = 15, xpd = TRUE, inset = c(0, -0.05)
            )
        } else {
            legend("bottomleft",
                legend = c("UP", "DOWN", "MIXED", "ID provided"), title = "Class feature and IDs",
    ...
    .heatplot.2(
        dataset = MX, Colv = as.dendrogram(Colv$dend), Rowv = as.dendrogram(Rowv$dend), margins = c(14, 12), cexRow = cex.names, cexCol = cex.names + m,
    ...
    if (all(!is.na(info.sample))) {
        legend("topleft",
            legend = col, col = names(col), pch = 15, cex = cex.legend - 0.2, title = "Sample info provided by user",
            bty = "n", xpd = TRUE, inset = c(0, 0.1)
        )
    }
    if (analysis == "Unsupervised") {
        legend("bottomleft",
            legend = c("Differential features", "ID provided features"), title = "Profile", col = c("black", "red"),
            bty = "n", cex = cex.legend - 0.1, pch = 15, xpd = TRUE, inset = c(0, -0.05)
        )
    } else {
        legend("bottomleft",
            legend = "ID provided", title = "Highlighted IDs", col = "red", bty = "n", cex = cex.legend + 0.1,
...
.plotRankingF <- function(deco, formatObj, info.size,
                          id, analysis,
                          default.layout) {
    layout(default.layout)
    ...
    if (analysis %in% c("Unsupervised", "Multiclass")) {
        color.table <- matrix(
            data = "black",
            nrow = min(info.size, dim(formatObj$textF)[1]),
        ...
        color.table[as.character(formatObj$textF[seq_len(info.size), "ID"]) == id, "ID"] <- "darkred"
        ...
        textplot(cbind(
            Ranking = seq_len(info.size),
            textF[seq_len(info.size), na.omit(match(formatObj$names.col, colnames(textF)))]
        ),
        ...
        for (i in seq_len(2))
        {
            ...
            n <- c(
                "sd.Ctrl", "Dendrogram.group.Ctrl", "h.Range.Ctrl",
            ...
            textF[seq_len(info.size), colnames(textF) %in% formatObj$names.col][
                is.na(textF[seq_len(info.size), colnames(textF) %in% formatObj$names.col])
            ] <- "NotAssigned"
            ...
            tex <- cbind(
                Ranking = seq_len(dim(textF)[1]),
                textF[, na.omit(match(formatObj$names.col, colnames(textF)))]
            )

            # Saving both top-50 features.
            IDS[[i]] <- na.omit(as.character(textF[which(
                textF$Profile %in% rank
            )[seq_len(dim(tex)[1])], c("ID")]))
            color.table <- matrix(
                data = "black",
                nrow = dim(tex)[1],
                ncol = length(which(colnames(deco@featureTable) %in% formatObj$names.col)),
                dimnames = list(rep(NA, dim(tex)[1]), colnames(textF))
            )
            if (all(is.na(id))) {
            ...
            color.table[, c("UpDw", "Profile")] <- unname(colors[unlist(
                textF[
                    seq_len(dim(tex)[1]),
            ...
            title(list(paste(
                ...
                c("MAJORITIES", "MINORITIES")[i],
                "ranking method was applied"
            ),
            cex = 1.5
            ), outer = TRUE, line = -2.5)
            legend("bottom",
                legend = c("RDA info", "NSCA: CONTROL info", "NSCA: CASE info"),
...
.plotColorGuide <- function(info.sample, infoS,
                            cex.legend, formatObj) {
    ...
    legend("center",
        legend = sort(colnames(infoS$orig)), cex = cex.legend + 0.7,
        title = "Sample categories provided by user", bty = "n", y.intersp = 1.3
    )
    plot(0, xlab = "", ylab = "", axes = FALSE, type = "n")
    legend("center",
        legend = infoS$ty, col = names(infoS$ty),
        pch = 15, cex = cex.legend + 0.7,
        title = "Sample info provided by user", bty = "n"
    )
    plot(0, xlab = "", ylab = "", axes = FALSE, type = "n")
    legend("center",
        legend = names(formatObj$color.cluster),
        col = formatObj$color.cluster, y.intersp = 1.3,
        pch = 15, cex = cex.legend + 0.7, xpd = TRUE,
        bty = "n", title = "Subclasses of samples found by DECO"
    )
    title(
        main = list(paste("Sample color code"), cex = 2.5, font = 2),
        line = -5, outer = TRUE
    )
    mtext(
    ...
    if (any(id %in% as.character(deco@featureTable[, c("ID")]))) {
        ...
        for (a in seq_len(length(id0)))
        {
            ...
            if (analysis == "Binary") {
                ...
                ord <- names(patt)
                bg.col <- adjustcolor(c(
                    rep("navyblue", table(deco@classes)[names(table(deco@classes)) == deco@control]),
                    rep("darkred", table(deco@classes)[names(table(deco@classes)) != deco@control])
                ), 0.6)
                limb <- list(
                    range(deco@NSCAcluster$Control$rankingFeature.h[, seq(2, dim(deco@NSCAcluster$Control$infoSubclass)[1] * 2, 2)]),
            ...
            plot(
                x = seq_len(dim(data)[2]), y = data[as.character(id0[a]), ], ylim = limy, pch = 21, xaxt = "n",
            ...
            axis(
                side = 1, at = seq_len(dim(data)[2]), labels = rownames(samplesSubclass[match(ord, samplesSubclass[, c("Samples")]), ]),
            ...
            if (all(!is.na(info.sample))) {
                ...
                for (i in seq_len(dim(cc)[2])) {
                    rect(0.5:(length(data[as.character(id0[a]), ]) - 0.5),
                        xright = 1.5:(length(data[as.character(id0[a]), ]) + 0.5), border = NA,
                        limy[1] - (m * 0.05 * i + 0.1 * m), ytop = limy[1] - (m * 0.05 * (i + 1) + 0.1 * m),
            ...
            rect(
                xleft = 0:(length(m) - 1), xright = seq_len(length(m)) - 0.1, ybottom = 0, ytop = m,
            ...
            segments(x0 = seq(0.45, length(m), 1) - 0.1, x1 = seq(0.45, length(m), 1) + 0.1, y0 = m + s, y1 = m + s)
            lab <- unlist(lapply(
                vapply(names(color.cluster), function(x) strsplit(x, split = " Subclass "), list(1)),
                function(x) paste(x, collapse = ".")
            ))
            axis(1,
                labels = lab, at = seq(0.45, length(m), 1),
            ...
            if (analysis == "Binary") {
                textplot(rbind(round(t(deco@featureTable[as.character(id0)[a], colnames(deco@featureTable) %in%
                    c(
                        "Standard.Chi.Square", "overlap.Ctrl.Case", "Repeats", "delta.signal",
                        "sd.Ctrl", "sd.Case", "h.Range.Ctrl", "h.Range.Case", "Dendrogram.group"
                    )]), 3),
                Profile = as.character(deco@featureTable[as.character(id0)[a], c("Profile")]),
                UpDw = as.character(deco@featureTable[as.character(id0)[a], c("UpDw")])
                ), halign = "right")
            }
            ...
            if (analysis %in% c("Binary", "Multiclass")) {
                overlapFeature(as.character(id0[a]), data0,
                    classes = deco@classes, analysis = analysis,
...
.plotDECObiplot <- function(deco, color.cluster, cex.samples) {
    ...
    for (i in seq_len(length(deco@NSCAcluster))) {
        ...
        s <- c()
        for (j in seq_len(length(color.cluster))) {
            centroid[j, ] <- colMeans(mx[
                rownames(samp[samp[, 1] %in% names(color.cluster)[j], ]),
                seq_len(2)
            ])
            s[j] <- mean(apply(mx[
                rownames(samp[samp[, 1] %in% names(color.cluster)[j], ]),
        ...
        points(centroid,
            cex = s / max(s) * 16, pch = 21,
            col = adjustcolor(color.cluster, 0.6), lwd = 4
        )
        ...
        legend("center",
            legend = c(
                paste(rownames(centroid), ", n =", deco@NSCAcluster[[i]]$infoSubclass[, "Samples"], "samples"),
        ...
        if (dim(mx)[2] >= 3) {
            scatterplot3d(mx[, seq_len(3)],
                bg = adjustcolor(samp[, "col"], 0.6), color = adjustcolor("grey", 0.3), mar = c(8, 8, 8, 8),
...
.plotRepThr <- function(sub, deco, id = NA, print.annot = FALSE) {
    if (all(is.na(id))) {
        id <- unique(c(as.character(deco@featureTable[seq_len(5), "ID"]), as.character(deco@featureTable[order(
            deco@featureTable$Repeats.index,
            decreasing = TRUE
        ), "ID"][seq_len(5)])))
    }
    ...
    g.names <- vapply(
        sort(rownames(sub$incidenceMatrix)), function(x) unlist(strsplit(x, split = "deco", fixed = TRUE))[1],
        character(1)
    )

    if (all(is.na(sub$classes)) || length(levels(sub$classes)) > 2) {
        x <- apply(
            sub$incidenceMatrix[sort(rownames(sub$incidenceMatrix)), ], 1,
            function(x) length(which(x > 0))
        ) / dim(sub$incidenceMatrix)[2]
        z <- apply(
            sub$incidenceMatrix[sort(rownames(sub$incidenceMatrix)), ], 1,
            function(x) length(which(x >= deco@rep.thr))
        ) / dim(sub$incidenceMatrix)[2]
        ...
        x <- vapply(unique(g.names), function(x)
            sum(apply(
                sub$incidenceMatrix[grepl(rownames(sub$incidenceMatrix), pattern = x, fixed = TRUE), ], 1,
                function(x) length(which(x > 0))
            )) / dim(sub$incidenceMatrix)[2], numeric(1))
    ...
    # Setting up Colors
    col <- vapply(
        y, function(j) max(which(seq(1, max(y), length.out = 16) <= j)),
        numeric(1)
    )
    ...
    # Plot
    layout(
        mat = matrix(c(1, 2, 3, 3), ncol = 2, byrow = TRUE),
        widths = c(1, 0.17), heights = c(1.3, 0.5)
    )
    par(mar = c(4, 10, 8, 2))
    plot(data,
        type = "p", pch = 21, col = adjustcolor("white", 0.1),
    ...
    legend("topleft",
        legend = paste(
            n, "features within <= ", round(deco@samp.perc, 3) * 100, "% samples with at least",
    ...
    title(
        main = list("RDA: Repeats threshold based on differential events distribution per feature", font = 2, cex = 1.4),
        outer = TRUE, line = -5
    )
...
.plotEffectSize <- function(deco, infoS = NA, samplesSubclass, analysis) {
    ...
    if (analysis == "Binary") {
        ...
        s <- c(
            sort(s[names(s) %in% names(deco@classes[deco@classes == deco@control])], decreasing = TRUE),
    ...
    axis(1,
        at = seq_len(length(s)), labels = rownames(samplesSubclass)[match(samplesSubclass[, 1], names(s))],
        tick = FALSE, las = 2
    )
    ...
    rect(seq_len(length(s)) - 0.35,
        xright = seq_len(length(s)) + 0.35, ybottom = 0, ytop = s,
        xlab = "", ylab = "",
    ...
    if (analysis == "Binary") {
        ...
        text(
            x = c(
                length(deco@classes[deco@classes == deco@control]) / 2,
                length(deco@classes[deco@classes != deco@control]) / 2 +
                    length(deco@classes[deco@classes == deco@control])
            ), font = 2, cex = 1.2,
    ...
    if (all(!is.na(infoS))) {
        ...
        for (i in seq_len(dim(infoS$col)[2])) {
            ...
            text(
                x = length(s) + 2, labels = colnames(infoS$col)[i],
                y = i - 0.5, adj = c(0, 0.5)
            )
        }
        plot(0, xlab = "", ylab = "", axes = FALSE, type = "n")
...
.plotHeterogeneityDECO <- function(v, var = 80, main = NA) {
    ...
    mtext(
        text = paste("Minimum dimensions to explain ",
            signif(var, 4), "% : ", thr,
            sep = ""
        ),
        las = 1
...
.plotTau <- function(x, samplesSubclass, labels = NA, n = 10, color.cluster,
    ...
    if (all(is.na(labels))) {
        labels <- vapply(
            names(sort(x[[1]]$NSCA$di, decreasing = TRUE)),
    ...
    for (j in seq_len(2))
    {
        if (length(x) == 2) {
            ...
            gene <- gene[names(sort(rowMeans(apply(gene, 2, rank)),
                decreasing = (j == 1)
            )[seq_len(n)]), ]
            rownames(gene) <- vapply(
                rownames(gene),
    ...
    for (i in seq_len(length(x)))
    {
        ...
        ## Drawing boxplot...
        .customBoxplot(dat,
            col = c(color.cluster[rownames(x[[i]]$infoSubclass)], "black"),
...
.plotRankingH <- function(deco, analysis, id = NA) {
    ...
    for (i in seq_len(length(deco@NSCAcluster))) {
        ...
        col <- c(
            rep("black", dim(deco@featureTable[rownames(d), colnames(deco@featureTable) %in% c("ID", "SYMBOL", "Standard.Chi.Square")])[2]),
        ...
        color.table[, which(grepl(colnames(d), pattern = "Ranking", fixed = TRUE))][
            apply(d[, which(grepl(colnames(d), pattern = "Ranking", fixed = TRUE))], 2, function(x) x %in% seq_len(10))
        ] <- "red"
        ...
        mtext(paste(
            "Top", min(c(50, dim(d)[1])),
            "discriminant features among subclasses found by DECO algorithm."
        ),
...
.plotOD <- function(deco, id, ord, symbol, print.annot = FALSE) {
    ...
    par(mar = c(10, 10, 1, 1))
    plot(s,
        pch = 21, xlab = "delta.signal", lwd = 2,
    ...
    legend("left",
        pch = c(16, 65, NA, 15, 15, 15, 15), lty = c(NA, NA, 2, NA, NA, NA, NA), pt.cex = 2.5, lwd = c(NA, NA, 2, NA, NA, NA, NA),
        legend = c(
...
plotRepThr <- function(sub, deco, id = NA, print.annot = FALSE) {
    ...
    g.names <- vapply(
        sort(rownames(sub$incidenceMatrix)), function(x)
    ...
    par(mar = c(4, 10, 8, 2))
    plot(x, y,
        type = "p", pch = 21, col = adjustcolor("white", 0.3), bg = col, axes = FALSE, cex = 2,
    ...
    for (a in seq_len(length(id)))
        text(data[grep(x = rownames(data), pattern = id[a], fixed = TRUE), ] - 0.01,
            labels = names[a], font = 2
        )
    legend("topleft",
        legend = c(
    ...
    mtext(paste("Calculated non-linear model, adjusted r-squared =", round(summary(m)$adj.r.squared, 3)),
        las = 1
    )
    title(
        main = list("RDA: Repeats threshold based on differential events distribution per feature", font = 2, cex = 1.4),
...
.plotDend <- function(dend, analysis, color.cluster, samplesSubclass, cex.names) {
    ...
    if (analysis == "Binary") {
        ...
        for (i in seq_len(2)) {
            ...
            plot(dend[[i]],
                axes = FALSE, cex = cex.names, lwd = 2, hang = -1, main = "",
                xlab = "", ylab = "", sub = ""
            )
            rect(0.5:(length(dend[[i]]$labels) - 0.5),
                xright = 1.5:(length(dend[[i]]$labels) + 0.5), border = NA,
                ybottom = -0.25 * max(dend[[i]]$height), ytop = -0.15 * max(dend[[i]]$height),
                col = col[dend[[i]]$labels[dend[[i]]$order]]
            )
            mtext(main[i], font = 2, cex = 1.2)
            mtext(
                side = 1, text = paste("Hubber's gamma coefficient for cutting dendrogram:", round(h[i], 3)),
        ...
        rect(0.5:(length(dend$labels) - 0.5),
            xright = 1.5:(length(dend$labels) + 0.5), border = NA,
    ...
    cols <- function() {
        if (lowcol == "green" & highcol == "red") {
            hmcol <- colorRampPalette(c(
                rev(RColorBrewer::brewer.pal(9, "Reds"))[seq_len(6)], "white",
                RColorBrewer::brewer.pal(9, "Greens")[4:9]
            ))(col.breaks)
        } else {
...
.heatmap.3 <- function(x,
    ...
    if (!missing(RowSideColors)) {
        if (!is.matrix(RowSideColors)) {
            ...
            text(
                ...
                x = 0, labels = length(table(rsc[, 1])):1,
    ...
    if (!missing(rowsep)) {
        for (rsep in rowsep) rect(
                xleft = 0, ybottom = (ncol(x) + 1 - rsep) - 0.5, xright = nrow(x) + 1,
...
.customBoxplot <- function(data, col = NA, ylab = NA, xlab = NA, ylim = NULL,
    ...
    {
        rect(
            ybottom = bdata[[i]]$stats[2, ], ytop = bdata[[i]]$stats[4, ], xleft = i - 0.25, xright = i + 0.25,
    ...
    if (paired.test) {
        ...
        for (i in seq_len(length(pos))) {
            ...
            x <- sort(c(x1, x0))
            segments(
                y0 = range(data)[2] + interval[i], y1 = range(data)[2] + interval[i],
                x0 = x[1], x1 = x[2], lwd = 2
            )
            segments(
                y0 = range(data)[2] + interval[i], y1 = range(data)[2] + interval[i] - diff(range(data)) * 0.01,
                x0 = x[1], x1 = x[1], lwd = 2
            )
            segments(
                y0 = range(data)[2] + interval[i], y1 = range(data)[2] + interval[i] - diff(range(data)) * 0.01,
...
.plotDECOmanhattan <- function(deco, id) {
    ...
    plot(deco@featureTable[, "Standard.Chi.Square"],
        xlab = "CHROMOSOME", xaxt = "n", ylab = "Standard Chi Square",
    ...
    text(
        x = which(n), font = 2, cex = 1.1,
        y = deco@featureTable[n, "Standard.Chi.Square"] +
            deco@featureTable[n, "Standard.Chi.Square"] * 0.05,
        labels = deco@featureTable[n, "SYMBOL"], xpd = NA, col = col
    )
    title(
        main = list("Manhattan plot showing Standard Chi Square values\nper feature and their chromosome locations", cex = 1.4, font = 2),
        outer = TRUE, line = -4
    )
}
    ...
    for (a in seq_len(length(id0)))
    {
        ...
        if (analysis == "Binary") {
            ...
            samplesSubclass <- rbind(
                cbind(deco@NSCAcluster$Control$samplesSubclass[order(deco@NSCAcluster$Control$samplesSubclass[, 1]), ]),
            ...
            count <- table(vapply(
                ...
                function(x) unlist(strsplit(x, split = " Subclass"))[1], character(1)
            ) != deco@control)
        ...
        if (!plot.h) {
            plot(data[as.character(id0[a]), ],
                ylim = limy, pch = pch, xaxt = "n", lwd = 0.8,
                xlim = c(1, ncol(data)), main = paste(main[a], "\nsorted by h-statistic ranking."),
            ...
            plot(data[as.character(id0[a]), ],
                ylim = limy, pch = 22, xaxt = "n", lwd = 1.2,
        ...
        if (analysis == "Binary") {
            ...
            segments(
                x0 = 1, x1 = dim(data)[2], y0 = mean(data[as.character(id0[a]), ]),
                col = adjustcolor("black", 0.4), lwd = 2
            )
        }
        if (print.annot) {
        ...
        d <- data.frame(samplesSubclass, h = tab.info[samplesSubclass[, "Samples"]])
        m <- vapply(
            names(color.cluster), function(x) mean(d[d[, "Subclass"] == x, "h"]),
            numeric(1)
        )
        s <- vapply(
            names(color.cluster), function(x)
        ...
        segments(x0 = seq(0.45, length(m), 1) - 0.1, x1 = seq(0.45, length(m), 1) + 0.1, y0 = m + s, y1 = m + s)
        axis(1,
            labels = rownames(infoSubclass), at = seq(0.45, length(m), 1),
            lty = 0, las = 2, cex.axis = 1.3
        )
        axis(2, las = 2, cex.axis = 1.5)
        ...
        if (analysis == "Binary") {
            plot(1, type = "n", axes = FALSE, xlab = "", ylab = "")
            legend("center",
                legend = paste(c("CONTROL", "CASE"), "samples"), col = "grey",
