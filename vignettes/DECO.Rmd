---
title: "DECO: DEcomposing heterogeneous Cohorts using Omic profiling data"
author:
...
library(knitr)
opts_chunk$set(warning=TRUE, message = FALSE, cache = TRUE, fig.path = "figures/", tidy = FALSE, tidy.opts = list(width.cutoff = 60))
```
...
BRCA_dataset_counts <- curatedTCGAData::curatedTCGAData(
    diseaseCode = "BRCA",
    assays = "BRCA_RNASeqGene", dry.run = FALSE
)

# or download normalized RNAseq data
BRCA_dataset_norm <- curatedTCGAData::curatedTCGAData(
    diseaseCode = "BRCA",
...
BRCA_dataset_counts_mirna <- curatedTCGAData::curatedTCGAData(
    diseaseCode = "BRCA",
    assays = "BRCA_miRNASeqGene", dry.run = FALSE
)
...
sub_binary <- decoRDA(
    ...
    parallel = TRUE, control = "pos", annot = FALSE,
...
sub_uns <- decoRDA(
    ...
    cpus = 6, iterations = 1000,
    parallel = TRUE, annot = FALSE, rm.xy = FALSE,
    id.type = "ENSEMBL", pack.db = "org.Hs.eg.db"
)
```
...
deco_results <- decoNSCA(
    sub = sub_binary, v = 80, method = "ward.D",
    k.control = 3, k.case = 3, samp.perc = 0.05, rep.thr = 5,
    parallel = TRUE, cpus = 6
)

deco_results_multiclass <- decoNSCA(
    sub = sub_multiclass, v = 80, method = "ward.D",
    k.control = 3, k.case = 3, samp.perc = 0.05, rep.thr = 5,
    parallel = TRUE, cpus = 6
)

deco_results_uns <- decoNSCA(
    sub = sub_uns, v = 80, method = "ward.D",
    k.control = 3, k.case = 3, samp.perc = 0.05, rep.thr = 5,
    parallel = TRUE, cpus = 6
)
```
...
A main result of DECO analysis is the *$h-statistic$ matrix* derived from both combination of RDA and NSCA information, which is described in the [original publication](#references) [1]. The `decoReport()` function generates the **heatmap** representation of this *$h-statistic$ matrix* that includes a double correlation analysis between samples and between features and two derived clustering dendrograms.
...
The **heatmap** reveals subclasses of samples and feature patterns, using the **h-statistic** as basis for this biclustering approach. Further information about all plots included in the PDF report (`decoReport()`) can be found using *help*.
