...
The **deco** R source package can be downloaded from [R CRAN repository](https://cran.r-project.org/) or from our lab website (http://bioinfow.dep.usal.es/deco/). It can be installed by downloading and executing in UNIX-like terminal:
...
biocLite(c("AnnotationDbi", "foreign", "limma", "Biobase", 
         "gdata", "cluster", "gplots", "RColorBrewer", 
         "snowfall", "scatterplot3d", "made4", "locfit", 
         "lisp", "ade4", "sfsmisc"))
...
Then, user can run *voom* normalization method provided in LIMMA R package to calculates matrix of **logCPMs**. Further information about **voom** normalization and its properties can be found in [LIMMA](#references) R package [2]. The normalized matrix is then analysed using the RDA method of DECO. 
...
sub_binary <- decoRDA(data = exprs(ALCLdata), classes = classes.ALCL, q.val = 0.05, iterations = 1000,
                      rm.xy = FALSE, r = NULL, cpus = 6, parallel = TRUE, temp.path = getwd(), 
...
\caption{Example of an "incidence matrix" obtained after RDA for a SUPERVISED analysis.}
\end{figure}
...
sub_uns <- decoRDA(data = exprs(ALCLdata), q.val = 0.05, r = NULL, cpus = 6, iterations = 1000,
...
\caption{Example of an "incidence matrix" obtained after RDA for a UNSUPERVISED analysis}
\end{figure}
...
sub_multiclass <- decoRDA(data = exprs(ALCLdata), classes = multiclasses.ALCL, q.val = 0.05, 
...
```{r, warning = FALSE}
dim(deco_results@featureTable)

# Statistics of top-10 features
deco_results@featureTable[1:10,]
```
...
sampleSubclass <- rbind(deco_results@NSCAcluster$Control$samplesSubclass, 
                        deco_results@NSCAcluster$Case$samplesSubclass)
# If UNSUPERVISED analysis
sampleSubclass <- deco_results@NSCAcluster$All$samplesSubclass
...
```{r, warning = FALSE, eval=FALSE}
### ERBB4 gene profile
plotDECOProfile(deco = deco_results, id = "ENSG00000178568", 
                data = exprs(ALCLdata), pdf.file = "ERBB4_ALCLdata.pdf",
                cex.samples = 2, info.sample = pData(ALCLdata)[,c(9,8,10)])
```
